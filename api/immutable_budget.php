...
try {
    ...
    if (!$isCreator && !$isMasterAdmin) {
        jsonError('Only trip creator or master admin can modify budget');
    }

    switch ($action) {
        case 'set':
            $budgetAmount = round(floatval($_POST['budget'] ?? 0), 2);
            $reason = trim($_POST['reason'] ?? 'Budget updated by user');
            ...
            try {
                ...
                if ($difference != 0) {
                    ...
                    $description = $reason ?: ($difference > 0 ?
